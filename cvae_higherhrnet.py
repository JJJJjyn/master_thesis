...
import _init_paths
from models.higher_hrnet import get_higherHRnet

from config import cfg
from config import update_config
from models.Hloss import MultiLossFactory
from models.trainer import do_train
from fp16_utils.fp16util import network_to_half
from fp16_utils.fp16_optimizer import FP16_Optimizer
from utils.utils_ import create_logger
from utils.utils_ import get_optimizer
from utils.utils_ import save_checkpoint
from utils.utils_ import setup_logger
...
def main_worker(
    ...
    model = get_higherHRnet(cfg, is_train=True)
    ...
    if cfg.FP16.ENABLED:
        ...
        for batch_id, data in enumerate(data_loader):
            inputs, y = data
            y_hat = model(inputs)
            train_data = TensorDataset(torch.tensor(inputs), torch.tensor(y_hat))
            ...
            for batch_id, data in enumerate(data_loader):
                ...
                y_ = model(x_gen)
                ltrain_data = TensorDataset(torch.tensor(x_gen), torch.tensor(y_))
                train_loader = DataLoader(train_data, batch_size=1, shuffle=True)
                do_train(cfg, model, train_loader, loss_factory, optimizer, epoch,
                         final_output_dir, tb_log_dir, writer_dict, fp16=cfg.FP16.ENABLED)
        ...
        # save imgs
          ...
          #  path = "./img/cvae_higherHRnet_model/epoch{}.png".format(epoch+1)
